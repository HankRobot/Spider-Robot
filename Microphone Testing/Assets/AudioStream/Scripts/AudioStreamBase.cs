...
namespace AudioStream
{
    public abstract class AudioStreamBase : MonoBehaviour
    {
        ...
        protected abstract void StreamStarting(int samplerate, int channels);
                ...
                if (!this.streamCaught)
                {
                    ...
                    do
                    {
                        ...
                        if (result == FMOD.RESULT.OK && openstate == FMOD.OPENSTATE.READY)
                        {
                            ...
                            this.StreamStarting((int)freq, this.streamChannels);
                    ...
                    else if (streamTag.type == FMOD.TAGTYPE.FMOD)
                    {
                        ...
                        if (streamTag.name == "Sample Rate Change")
                        {
                            ...
                            if (this is AudioStream)
                            {
                                ...
                                if (audiosrc != null)
                                    audiosrc.pitch = (float)(frequency[0] * this.streamChannels) / (float)(AudioSettings.outputSampleRate * AudioStreamSupport.ChannelsFromAudioSpeakerMode(AudioSettings.speakerMode));
                            }
                        }
                    }
                }
